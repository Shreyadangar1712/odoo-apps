...
class MaintenanceRequest(models.Model):
    ...
    property_id_itys = fields.Many2one(
        'building.unit',
        string='Property/Unit',
        help='Building unit associated with this maintenance request'
    )
...
class building_unit(models.Model):
    ...
    special_property= fields.Boolean('Special Property')
    have_maintenance = fields.Boolean('Have Maintenance')
    ...
    def view_maintenance(self):
        ...
        maintenance_obj = maintenance_obj.search([('property_id_itys', '=', self.ids)])
        reservations = []
        for obj in maintenance_obj: reservations.append(obj.id)
        return {
            'name': _('Maintenance'),
            'domain': [('id', 'in', reservations)],
            'view_type':'form',
    ...
    def _maintenance_count(self):
        maintenance_obj = self.env['maintenance.request']
        for unit in self:
            maintenance_obj = maintenance_obj.search([('property_id_itys', '=', unit.id)])
            if maintenance_obj:
                self.have_maintenance = True
            else:
                self.have_maintenance = False
            unit.maintenance_count = len(maintenance_obj)
