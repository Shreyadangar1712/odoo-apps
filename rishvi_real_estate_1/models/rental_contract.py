...
class rental_contract(models.Model):
    ...
    def _default_security_deposit_account(self):
        security_deposit_account= self.env['ir.config_parameter'].sudo().get_param('rishvi_real_estate.security_deposit_account')
        return int(security_deposit_account)

    def _default_income_account(self):
        account_income= self.env['ir.config_parameter'].sudo().get_param('rishvi_real_estate.revenue_account')
        ...
        if self.building:
            ...
            if building_obj:
                return {'value': {
                        ...
                        'domain': {'building_unit': [('id', 'in', unit_ids)], 'building_unit_ids': [('id', 'in', unit_ids)], }}
        ...
        for rec in self:
            if rec.periodicity and rec.date_from and rec.date_to:
                ...
                if self.insurance_fee:
                    rental_lines.append(
                        (0, 0, {'serial': i, 'company_id':self.env.company.id,'amount': self.insurance_fee, 'date': date_from, 'name': _('Insurance Deposit')}))
                    i += 1
                rental_lines.append((0, 0, {'serial': i,'company_id':self.env.company.id, 'amount': rental_fee, 'date': date_from, 'name': _('Rental Fee')}))
                i+=1
                ...
                while new_date < (date_to - relativedelta(**{periodicity:self.recurring_interval})):
                    ...
                    rental_lines.append((0, 0, {'serial': i,'company_id':self.env.company.id, 'amount': rental_fee, 'date': new_date, 'name': _('Rental Fee')}))
                    i+=1
