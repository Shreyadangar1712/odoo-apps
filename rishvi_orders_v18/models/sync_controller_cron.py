...
class SyncControllerCronOrder(models.Model):
    _name = 'sync.controller.cron.order'
    _description = 'Scheduled Action to Call Controller'



    def sync_bulk_order(self):
        """
        ...
        try:
            ...
            pages = (total_order_count // 30) + (1 if total_order_count % 30 > 0 else 0)
            for page in range(1, pages + 1):
                self.sync_all_the_order(page_number=page)
    ...
    @api.model
    # def cron_call_controller_sync_orders(self):
    def sync_all_the_order(self, **kwargs):
        ...
        try:
            ...
            rishvi_base_api_url = icp.get_param("rishvi_base_api_url")
            rishvi_app = icp.get_param("rishvi_app")
            params = {"page": page_number, "limit": 30, "appName": rishvi_app, "appToken": lw_token}
            headers = {"accept": "*/*"}
    ...
    def _process_order_with_workflow(self, order_data, auto_post_invoice,
        ...
        # Workflow flags
        lw_status = order_data.get('status', 0)
        ...
        for item in items:
            ...
            order_line_vals.append({
                ...
                'tax_id': [(6, 0, tax.ids)]  # Add taxes here
            })
        ...
        if existing_order:
            ...
            if status == 1:  # Post invoice for PAID orders
            ...
            try:
                ...
                order.notes = order_data.get('notes')[0]['note'] if order_data.get('notes') and len(
                    order_data.get('notes')) > 0 else " "
    ...
    @http.route('/order/status_counts_status', type='json', auth='user')
