...
class jewelleryAddons(models.Model):
        ...
        def _compute_total_metal(self):
            ...
            for rec in self:
                ...
                tax = rec.taxes_id[:1]
                gst_percentage = tax.amount if tax else 0.0
                gst_value = round((gst_percentage / 100.0) * (rec.list_price or 0.0), 2)
                ...
                if rec.show_price:
                    price_breakup_string = self.apply_discount_to_price_breakup(
                        price_breakup,
                        rec.discount_stone_price,
                        rec.discount_making_price,
                        gst_percentage,
                        rec.discount_making_percentage,
                    )
