...
class SaleOrderReportXlsx(models.AbstractModel):
    _name = "report.rishvi_hotel_management_system.report_sale_order_xlsx"
    _inherit = "report.report_xlsx.abstract"

    def generate_xlsx_report(self, workbook, data, obj):
        """Generate XLSX report for Sale Order"""

        _logger.info("=== SALE ORDER XLSX REPORT START ===")
        _logger.info(f"Data received: {data}")

        # Parse the JSON string from options if it exists
        if data.get("options"):
            try:
        ...
        for record in booking_data:
            ...
            sheet.write(row, 2, str(checkin), body)
            sheet.write(row, 3, str(checkout), body)
            sheet.write(row, 4, str(ref_no), body)
            sheet.write(row, 5, "{:.2f}".format(float(amount)), body)

            row += 1
            value += 1

        _logger.info("=== SALE ORDER XLSX REPORT END ===")