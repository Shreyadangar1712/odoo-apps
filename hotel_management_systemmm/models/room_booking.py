...
class RoomBooking(models.Model):
    ...
    room_type = fields.Selection(
        selection=[('single', 'Single'),
                   ...
                   ('dormitory', 'Dormitory')],
        string="Room Type",
        help="Choose the type of the room")
    ...
    def get_details(self):
        ...
        for rec in check_outs:
            ...
            staff = self.env['res.users'].search_count(
                [('groups_id', 'in',
                  [self.env.ref('rishvi_hotel_management_system.hotel_group_admin').id,
                   self.env.ref(
                       'rishvi_hotel_management_system.cleaning_team_group_head').id,
                   self.env.ref(
                       'rishvi_hotel_management_system.cleaning_team_group_user').id,
                   self.env.ref(
                       'rishvi_hotel_management_system.hotel_group_reception').id,
                   self.env.ref(
                       'rishvi_hotel_management_system.maintenance_team_group_leader').id,
                   self.env.ref(
                       'rishvi_hotel_management_system.maintenance_team_group_user').id
                   ])])
