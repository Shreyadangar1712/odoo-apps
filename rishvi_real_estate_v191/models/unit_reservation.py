...
class unit_reservation(models.Model):
    ...
    def auto_cancel_reservation(self):
        try:
            ...
            reservation_hours = int(self.env['ir.config_parameter'].sudo().get_param('rishvi_real_estate.reservation_hours'))
    ...
    def action_contract_ownership(self):
        ...
        return {
            ...
            'view_mode': 'form',
            'view_id': self.env.ref('rishvi_real_estate.ownership_contract_form_view').id,
            'target': 'current',
            'context': {
                'default_building': self.building.id or False,
                'default_region': self.region.id or False,
                'default_building_code': self.building_code or False,
                'default_partner_id': self.partner_id.id or False,
                'default_building_unit': self.building_unit.id or False,
                'default_unit_code': self.unit_code or False,
                'default_floor': self.floor or False,
                'default_type': self.type.id or False,
                'default_status': self.status.id or False,
                'default_building_area': self.building_area or 0.0,
                'default_reservation_id': self.id,
                'form_view_initial_mode': 'edit',
            },
        }
    ...
    def action_contract_rental(self):
        ...
        return {
            ...
            'view_mode': 'form',
            'view_id': self.env.ref('rishvi_real_estate.rental_contract_form_view').id,
            'target': 'current',
            'context': {
                'default_building': self.building.id or False,
                'default_region': self.region.id or False,
