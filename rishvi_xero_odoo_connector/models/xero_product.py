...
class XeroProductProduct(models.Model):
    ...
    def prepare_product_export_dict(self):
        ...
        if self.property_account_expense_id:
            if self.property_account_expense_id.xero_account_id:
                ...
                if self.is_storable and not xero_config.non_tracked_item:
                    purchase_dict = {
    ...
    def create_product_in_xero(self):
        ...
        products = self.search([
            ('xero_product_id', '=', False)
        ])

        _logger.info("PRODUCTS TO EXPORT: %s", products)
        _logger.info("TOTAL PRODUCTS: %s", len(products))

        for product in products:
            _logger.info("EXPORTING PRODUCT: %s (%s)", product.name, product.id)
    ...
    def create_main_product_in_xero(self, p, xero_config):
        _logger.info("helloooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo")
