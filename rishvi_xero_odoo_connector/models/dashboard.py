...
class XeroPurchaseOrder(models.Model):
    ...
    @api.model
    def fetch_purchase_id(self, element):
        purchase_id = []
        today = DT.date.today()
        week_ago = today - DT.timedelta(days=7)
        month_ago = today - DT.timedelta(days=30)
    ...
    def purchase_piechart_month_detail(self):
        ...
        if purchaseCountTotal == 0:
            ...
            dataPoints = [['State', 'Mhl'],
                          ['Draft', 100 * draftPurchaseCount / purchaseCountTotal],
                          ['Purchase', 100 * totalPurchaseCount / purchaseCountTotal],
                          ['Done', 100 * donePurchaseCount / purchaseCountTotal]]
...
class XeroSaleOrder(models.Model):
    _inherit = "sale.order"
...
class XeroAccountMove(models.Model):
