...
class XeroTax(models.Model):
    ...
    isApplicableToEquity = fields.Boolean(string="Can Apply To Equity",default=False)
    isApplicableToLiabilities = fields.Boolean(string="Can Apply To Liabilities",default=False)
    isApplicableToRevenue = fields.Boolean(string="Can Apply To Revenue ",default=False)
    isApplicableToExpenses = fields.Boolean(string="Can Apply To Expenses",default=False)
    isApplicableToAssets = fields.Boolean(string="Can Apply To Assets",default=False)
    ...
    #     #     account = self.browse(self.env.context.get('active_ids'))
    #     # else:
    #     account = account_id
    ...
    def prepare_tax_export_dict(self):
        ...
        if self.name:
            if self.isApplicableToEquity:
                if self.isApplicableToEquity == True:
                        ApplyToEquity = 'true'
                else:
                        ApplyToEquity = 'false'

            if self.isApplicableToLiabilities:
                if self.isApplicableToLiabilities == True:
                        ApplyToLiabilities = 'true'
                else:
                        ApplyToLiabilities = 'false'

            if self.isApplicableToRevenue:
                if self.isApplicableToRevenue == True:
                        ApplyToRevenue = 'true'
                else:
                        ApplyToRevenue = 'false'

            if self.isApplicableToExpenses:
                if self.isApplicableToExpenses == True:
                        ApplyToExpenses = 'true'
                else:
                        ApplyToExpenses = 'false'

            if self.isApplicableToAssets:
                if self.isApplicableToAssets == True:
                        ApplyToAssets = 'true'
                else:
                        ApplyToAssets = 'false'
            ...
            if self.amount_type:
                if self.amount_type == 'percent':
                    ...
                    vals.update({
                        ...
                        "isApplicableToEquity": ApplyToEquity,
                        "isApplicableToLiabilities": ApplyToLiabilities,
                        "isApplicableToRevenue": ApplyToRevenue,
                        "isApplicableToExpenses": ApplyToExpenses,
                        "isApplicableToAssets": ApplyToAssets,
                    })
    ...
    def create_tax_in_xero(self):
        ...
        xero_config = self.env['res.users'].search([('id', '=', self.env.uid)], limit=1).company_id
        if self.env.context.get('active_ids'):
            tax = self.browse(self.env.context.get('active_ids'))
        else:
            tax = self
