...
class XeroCustomer(models.Model):
    ...
    def create_customer_in_xero(self):
        ...
        _logger.info("3333333333333333333333333333333333333333333333")

        if not company:
            raise UserError(
                _("No company found for export. Please select a company or set a default company.")
            )

        # Customers not yet exported to Xero
        partners = self.search([
            ('xero_cust_id', '=', False),
        ])
        _logger.info("3333333333333333333333333333333333333333333333")

        _logger.info("CUSTOMERS TO EXPORT: %s", len(partners))

        for partner in partners:
            _logger.info(
                "EXPORTING CUSTOMER: %s (ID: %s)",
                partner.name,
                partner.id
            )
            self.create_main_customer_in_xero(partner, company)

        success_form = self.env.ref(
            'rishvi_xero_odoo_connector.export_success_view',
            False
        )
    ...
    #         partner = self.browse(self.env.context.get('active_ids'))
    #     else:
    #         partner = self
    #
    #     for con in partner:
