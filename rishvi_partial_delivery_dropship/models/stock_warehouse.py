...
class StockWarehouse(models.Model):
    ...
    mts_mto_rule_id = fields.Many2one(
        "stock.rule", "MTO+MTS rule", check_company=True
    )
    ...
    def _get_global_route_rules_values(self):
        ...
        res.update(
            {
                "mts_mto_rule_id": {
                    "depends": ["delivery_steps", "split_stock_purchase"],
                    "create_values": {
                        "action": "pull",
                    ...
                    "update_values": {
                        "active": self.split_stock_purchase,
            ...
            rule = self.env["stock.rule"].search(
                [
                    (
                        "location_dest_id",
                        "=",
                        self.mts_mto_rule_id.location_dest_id.id,
                    ),
                    (
                        "location_src_id",
                        "=",
